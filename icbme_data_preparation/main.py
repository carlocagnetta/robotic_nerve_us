...
def create_batch_file(image_path, output_folder, batch_file_path, num_samples=100):
    ...
    np.savetxt(os.path.join(output_folder, 'array.txt'), samples, delimiter=' ', fmt='%.2f')

    close_samples, nearest_points = find_liver_points(image_data, padded_image, samples)

    # Save the array to a file
    np.savetxt(os.path.join(output_folder, 'close_points.txt'), close_samples, delimiter=' ', fmt='%.2f')
    ...
    print(f'Created {num_lines} lines in the batch file.')
...
if __name__ == '__main__':
    ...
    args.batch_file_path = os.path.join(args.output_folder, 'batch.txt')
